...
function togglePhrase(k,btn){
  const i=S.phrases.indexOf(k);
  const saved=i<0;
  if(saved)S.phrases.push(k);else S.phrases.splice(i,1);
  save();updHd();

  // حدّث جميع نسخ زر العبارة نفسها دون إعادة بناء الشاشة كاملة.
  document.querySelectorAll('.fav[data-fav-key]').forEach(b=>{
    if(b.dataset.favKey!==k)return;
    b.classList.toggle('on',saved);
    b.innerHTML=saved?ic('star'):ic('starOutline');
  });

  // داخل شاشة العبارات المحفوظة: احذف البطاقة فقط عند إلغاء الحفظ.
  if(V==='phrases'&&!saved){
    const box=(btn||document.activeElement)?.closest?.('.favbox');
    if(box)box.remove();
    const view=document.getElementById('v-phrases');
    if(view&&!view.querySelector('.favbox')){
      view.insertAdjacentHTML('beforeend',`<div class="c empty">اضغط ${ic('starOutline')} بجانب أي جملة لحفظها هنا.</div>`);
    }
  }
}
function favBtn(k){return `<button class="fav ${phraseSaved(k)?'on':''}" data-fav-key="${esc(k)}" onclick="event.stopPropagation();togglePhrase('${k}',this)" title="حفظ العبارة">${phraseSaved(k)?ic('star'):ic('starOutline')}</button>`}
...
const PHRASE_EXTRA={};
function favExtraBtn(k){const sk='x:'+k;return `<button class="fav ${phraseSaved(sk)?'on':''}" data-fav-key="${esc(sk)}" onclick="event.stopPropagation();togglePhrase('${sk}',this)">${phraseSaved(sk)?ic('star'):ic('starOutline')}</button>`}
...
function vPhrases(){
 ...
 h+=list.length?list.map(x=>`<div class="favbox"><div style="display:flex;justify-content:space-between;gap:8px"><div><b class="en">${esc(x.en)}</b><div>${esc(x.ar)}</div><div class="sub">${esc(x.m||'')}</div></div><div>${spkBtn(x.en)} <button class="fav on" data-fav-key="${esc(x.k)}" onclick="togglePhrase('${x.k}',this)">${ic('star')}</button></div></div></div>`).join(''):`<div class="c empty">اضغط ${ic('starOutline')} بجانب أي جملة لحفظها هنا.</div>`;
...
function updHd(){
  ...
  countTo(document.getElementById('h-xp'),S.phrases.length);
